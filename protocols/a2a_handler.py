...
class A2AHandler:
    ...
    def submit_task(self, sender: str, receiver: str, content: str, task_id: str = None) -> A2ATask:
        if task_id and task_id in self.tasks:
            ...
            task.messages.append(A2AMessage(sender=sender, receiver=receiver, content=content))
            return self.route_task(task)
        else:
            task = A2ATask.create(sender, receiver, content)
            if task_id:
                task.task_id = task_id  # wymuś użycie przekazanego ID
            ...
            if agent:
                ...
                if response.get('status') == 'forward':
                    ...
                    if forward_to and forward_content:
                        # Dodaj forward_content tylko jeśli różni się od poprzedniej odpowiedzi
